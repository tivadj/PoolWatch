...
namespace PoolWatch
{
	...
	bool distLinePoint(cv::Point2f p1, cv::Point2f p2, cv::Point2f freePoint, float& dist)
	{
		...
		float a = p1.y - p2.y;
		float b = -(p1.x - p2.x);
		float nNorm = std::sqrtf(a*a + b*b);
		
		// two points p1 and p2 must be different
		if (nNorm < 1e-6)
			return false;
		...
		//
		dist = std::fabs(a * freePoint.x + b * freePoint.y + d);
		dist /= nNorm;
		return true;
	}
}
...
void classifyAndGetGrayMask(const cv::Mat& image, std::function<uchar(const cv::Vec3d&)>  pred, cv::Mat_<uchar>& mask)
{
	assert(image.channels() == 3);
	assert(image.depth() == CV_8U);
