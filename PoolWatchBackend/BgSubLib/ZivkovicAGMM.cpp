...
void ZivkovicAGMM::loadGmmState(const ZivkovicAGMM::GMM* gmms, int gmmsCount)
{
	assert(gmmsCount <= m_params.MaxModes());
	assert(1 == m_params.Size());

	for (unsigned int iPix = 0; iPix < m_params.Size(); ++iPix)
	{
		m_modes_per_pixel[iPix] = gmmsCount;

		int globalModeInd = iPix * m_params.MaxModes();

		for (unsigned int iMode = 0; iMode < gmmsCount; ++iMode)
		{
			m_modes[globalModeInd].weight = gmms[iMode].weight;
			m_modes[globalModeInd].sigma = gmms[iMode].sigma;
			m_modes[globalModeInd].muR = gmms[iMode].muR;
			m_modes[globalModeInd].muG = gmms[iMode].muG;
			m_modes[globalModeInd].muB = gmms[iMode].muB;
...
void ZivkovicAGMM::saveGmsState(ZivkovicAGMM::GMM* gmms, int gmmsMaxSize, int& newGmmSize)
{
	assert(gmmsMaxSize <= m_params.MaxModes());
	assert(1 == m_params.Size());

	for (unsigned int iPix = 0; iPix < m_params.Size(); ++iPix)
	{
		newGmmSize = m_modes_per_pixel[iPix];

		int globalModeInd = iPix * m_params.MaxModes();

		for (unsigned int iMode = 0; iMode < newGmmSize; ++iMode)
		{
			gmms[iMode].weight = m_modes[globalModeInd].weight;
			gmms[iMode].sigma = m_modes[globalModeInd].sigma;
			gmms[iMode].muR = m_modes[globalModeInd].muR;
			gmms[iMode].muG = m_modes[globalModeInd].muG;
			gmms[iMode].muB = m_modes[globalModeInd].muB;
